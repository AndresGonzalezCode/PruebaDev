    ...
    if (!empty($arrCliente)) {
        for ($i = 0; $i < count($arrCliente); $i++) {
            ...
            if ($arrCliente[$i]["estado"] == "A") {
                ...
                $act = 1;
                $inactivo = '<button class="btn btn-icon btn-sm btn-primary" onClick="InactivarUsuario(' . $arrCliente[$i]['id'] . ','. "$act" .')" title="Habilitar Cliente"><i class="bx bxs-message-square-x"></i></button>';
            }
